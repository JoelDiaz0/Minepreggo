...
T extends AbstractZombieGirlInventaryGUIMenu<E>> extends AbstractContainerScreen<T> {
	...
	protected AbstractZombieGirlInventaryGUIScreen(T container, Inventory inventory, Component text) {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (zombieGirl != null) {
			InventoryScreen.renderEntityInInventoryFollowsAngle(guiGraphics, this.leftPos + 51, this.topPos + 78, 40, 0f + (float) Math.atan((this.leftPos + 51 - mouseX) / 40.0), (float) Math.atan((this.topPos + 29 - mouseY) / 40.0), zombieGirl);
		}
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int gx, int gy) {
		...
		guiGraphics.blit(PreggoGUIHelper.ZOMBIE_GIRL_INVENTARY_TEXTURE, this.leftPos, this.topPos, 0, 0, this.imageWidth, this.imageHeight, this.imageWidth, this.imageHeight);
