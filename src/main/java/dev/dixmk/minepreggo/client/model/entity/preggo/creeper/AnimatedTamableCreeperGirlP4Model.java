...
@OnlyIn(Dist.CLIENT)
public class AnimatedTamableCreeperGirlP4Model extends AbstractTamablePregnantCreeperGirlModel<TamableCreeperGirlP4> {
	
	public AnimatedTamableCreeperGirlP4Model(ModelPart root) {
		...
		return new HierarchicalModel<E>() {		
			...
			public void setupAnim(E creeperGirl, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
				...
				if (creeperGirl.isIncapacitated()) {
					...
					case PREBIRTH: {
						this.animate(creeperGirl.loopAnimationState, ZombieGirlAnimation.BIRTH, ageInTicks, 1f);						
						break;
					}
					case BIRTH: {
						this.animate(creeperGirl.loopAnimationState, ZombieGirlAnimation.MISCARRIAGE, ageInTicks, 1f);						
					...
					case KICKING: {
						this.animate(creeperGirl.loopAnimationState, ZombieGirlAnimation.KICKING1, ageInTicks, 1f);						
						break;
					}
					case CONTRACTION: {
						this.animate(creeperGirl.loopAnimationState, ZombieGirlAnimation.CONTRACTION, ageInTicks, 1f);						
						break;
					}
					default:
						break;						
